...
describe("createRunWorkflowCommand", () => {
  ...
  test("calls workflow.run and shows progress toast when workflow is selected", async () => {
    ...
    const toastMsg = (showProgressToast as ReturnType<typeof vi.fn>).mock.calls[0][0] as string;
  ...
  test("passes workflow name (not description) in label for quick pick", async () => {
    ...
    await cmd();
    const picksArg = (showQuickPick as ReturnType<typeof vi.fn>).mock.calls[0][0] as Array<{
      label: string;
      description: string;
    }>;
