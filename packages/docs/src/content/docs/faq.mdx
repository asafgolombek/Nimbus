...
Metadata only in SQLite (titles, paths, IDs, sync cursors, audit entries, embeddings metadata). **Raw email bodies and full file contents are not bulk-stored** in the index by design — see the repository `architecture.md` for the exact boundary.
