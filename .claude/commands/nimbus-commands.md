...
bun run audit:invariants                # static invariant complement D10–D28 (spawn rule, vault-key allow-list, SQL writes, federation/identity/team-vault gates, policy, chatops, preflight, tribal, connector writes, share, egress dispatch chokepoint, exec runConfined, computer-use actuation/driver/lane, remote-VLM constructor + media_grant table, fleet ClientKind) — derive the range from `check-nimbus-invariants.ts`; see CLAUDE.md § Security Invariants
