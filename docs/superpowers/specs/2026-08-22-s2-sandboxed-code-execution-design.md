...
4. **The child inherited no environment, and on Windows that stopped it existing.** `runConfined` passed `env: {}` — deliberately, so an execution inherits nothing that could be exfiltrated. But `CreateProcessW` fails with 203 (`ERROR_ENVVAR_NOT_FOUND`) when the child has no `SystemRoot`, so on Windows EVERY execution died at exit 68 before running a line. Now uses `extensionProcessEnv({})`, the same I1-governed allowlist every spawned MCP child already uses — same intent, and not a second private answer to a question this tree had already answered.

5. **The policy never granted the interpreter's own directory.** Linux hid it (bwrap binds the system tree by default); Windows AppContainer writes an ACE per granted path, so an ungranted `bun.exe` is simply unreadable. `ExecRuntime.requiredReadPaths()` now supplies it, keeping the knowledge with the runtime — including that macOS additionally needs the runtime home while Windows must NOT get it, since `~/.bun` carries thousands of cache entries and granting it makes every spawn hang.

6. **The body is passed INLINE (`bun -e`); there is no scratch file, and §4's whole scratch-script design is gone.** This is what makes Windows work, and it took bisecting the sandbox to find. Under the AppContainer, bun can read a granted file, list its cwd, report `process.cwd()`, and even `import()` and execute that very file — but naming a file as its **entry point** (`bun s.ts` / `bun run s.ts`) dies with `CouldntReadCurrentDirectory`. Something in bun's startup path for a file entry point touches what the sandbox denies; `-e` does not. Neither env completeness (an 81-key environment failed identically) nor 8.3 short names (the paths were byte-identical) nor ancestor traversal (granting the parent changed nothing, and granting `tmpdir()` reproduced the ACE-explosion hang) was the cause.

   The inline form is better on its own terms, not merely a workaround: **no file is written anywhere**, so nothing can leak on a denial, race the spawn, or be read or swapped by another local user between approval and execution. The read-once rule from §4 becomes structural — the approved bytes go straight to the interpreter with no file in between. The cost is a bound: a body must fit `MAX_INLINE_CODE_UNITS` (16,384), refused with a named error before consent rather than truncated, because running a *prefix* of someone's script is worse than refusing all of it.

   **Windows and Linux are both verified**: 5/5 integration and 5/5 e2e on Windows against a real AppContainer, 5/5 integration on Linux against real `bwrap`, loopback blocked on both.

7. **`--allow-fs-write` did not work, and no unit test could have caught it.** The connector-tuned seccomp filter omits `ftruncate`, so `fs.writeFileSync` and `Bun.write` were SIGSYS-killed with **no stderr at all** — the sandbox permitting a write that then died silently. Isolated by bisection, not guessed. Nothing short of running against a real sandbox on Linux would have found it, which is the argument for §8's integration row being non-optional.
