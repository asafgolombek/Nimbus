...
- [ ] **Settings** — ~~model selection (cloud vs local)~~ ✅ (Plan 3 — `RouterStatus` + `PullDialog`) · ~~sync intervals per connector~~ ✅ (Plan 3 — `ConnectorsPanel` with depth + enable + 60 s-min interval) · ~~profile switcher~~ ✅ (Plan 2) · Vault key listing (no values shown) · audit log viewer + export · data export/import · ~~telemetry toggle~~ ✅ (Plan 2) — Plans 4–5 complete Audit / Data / Updates panels
