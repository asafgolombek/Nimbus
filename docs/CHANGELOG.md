...
- **2026-09-06 — the multimodal remote arm was answered by a real vendor for the first time.**
  ...
  documented image API and tested only against fakes, so every claim about them proved the shape
  this code **sends** and none proved that a vendor **accepts** it. Run on merged `main` at
  `580e8930`, built from source, against a real Gemini key held in the Vault and a real
  390,842-byte Google Drive PNG, in the `LOCALAPPDATA`-override sandbox (real config, real Vault,
  real outbound traffic, throwaway index): Gemini returned a caption; the pass appended **exactly
  one** `model`-class ledger row (`destination: gemini`, `method: multimodal.vlm.image`,
  `payload_summary` `{"model":"gemini-3.5-flash","imageBytes":390842}` — the model name and the
  byte **count**, never the prompt and never the bytes); the derived row recorded
  `model: "gemini/gemini-3.5-flash"` and `isLocal: false`; `expectedBytes` rode the cloud-fetch
  row; and `verifyEgressChain` was ok over 65 rows. The remote caption was also visibly better,
  which is the feature's whole premise — the local `qwen2.5vl:7b` called the artifact "a knife",
  while Gemini identified a long pole weapon with a curved blade and named the scarf joint, the
  owner's-mark band and the pledge tag. **It narrows the gap rather than closing it:** Anthropic
  and OpenAI remain fixture-only, as does every failure mode on all three (a 400 on a bad
  `media_type`, a 429, an auth refusal), the Google Photos leg (still an empty library under
  `photoslibrary.readonly`), OneDrive entirely, and the TTY confirmation path — that last one
  because `nimbus media allow-remote` **correctly refused a piped `y`** in non-TTY mode, so the
  run's grant was created through the LAN-forbidden `media.allowRemote` IPC method with the
  owner's approval obtained out of band. One defect surfaced and is filed rather than fixed: the
...
- **2026-09-04 — `GET /v1/items/resolve-file`: the forge-file read the browser client already
  ...
  change, no new invariant. **Bound, stated rather than glossed:** only a checkout of a
  `github.com`/`gitlab.com`/`bitbucket.org` remote with a two-segment `owner/name` path can ever
