...
**Brief synthesis.** Every built-in agent's Markdown brief can be rewritten by a local LLM (Ollama or llama.cpp) into more natural prose before it reaches you, instead of the deterministic template render. With Ollama running, this happens **by default** — set `synthesis = "off"` below to keep the deterministic-only behaviour. Two honesty guards protect a rewrite from saying less than the deterministic render promised (invariant **I31**). Disclosure-only sections — `## Gaps` for all fourteen brief kinds, plus `nimbus negotiate`'s `## Sources` and `## Evidence not available from the index` — are never shown to the model at all: the renderer builds them and re-attaches them verbatim afterwards, so a rewrite cannot drop one by construction. Disclosures woven into prose the model *is* meant to rewrite are covered instead by a `requiredPhrases` anchor check, which spans the whole interleaved set — `negotiate`'s seven null-lane disclaimers, its window, ownership-accountability, list-truncation and `unattributable` clauses, and `glossary`'s two definition-provenance lines. A rewrite that drops any of them is discarded and the deterministic render is served instead. Each sentence and its anchor have a single definition in `agents/_lib/brief-disclosures.ts`, read by both the renderer and the guard, so the two cannot drift apart. A rendered brief always carries a plain-language footer stating its own provenance: deterministic by design (`synthesis = "off"`, or no LLM registry wired), deterministic because no eligible provider was resolved, deterministic after an attempt was made and then discarded (with the reason), or synthesized (with the model name and whether it was local or remote) — and the same fact rides the `briefReady` notification and `GET /v1/agents/runs/{id}` as a machine-readable `synthesis` field.

**Configuration — `[agents]`, read from the profile-resolved TOML:**

`[agents]` follows the same **whole-file** profile semantics as `[llm]` and `[session]`: when a profile is active and its `nimbus.<profile>.toml` exists, that file is the ONLY one read, and the base `nimbus.toml` is not consulted for these keys. This cuts **both ways**, and both directions are real behaviour changes as of 2026-08-18. A key set in a profile TOML is now honoured, where it used to be silently ignored; and a key set ONLY in the base `nimbus.toml` no longer applies while a profile whose file lacks an `[agents]` section is active — so a user with `synthesis = "off"` in `nimbus.toml` and an `[agents]`-less `nimbus.work.toml` gets the `"local"` default under the `work` profile until they repeat the key there.
...
**Configuration — `[persona]`, read from the profile-resolved TOML (see below):**
...
`[persona]` is read from the **profile-resolved** TOML and resolved fresh on every query — editing it changes the very next response, no restart required. Profile resolution means: the active profile's `nimbus.<profile>.toml` when a profile is active **and that file exists**, otherwise the base `nimbus.toml`. Switching *which* profile is active (`nimbus profile switch`) still requires a Gateway restart, same as every other profile-scoped setting, because the active profile is fixed for the life of the Gateway process.

Applies to **three** surfaces: `nimbus ask`, every built-in agent brief, and — because a `@nimbus <question>` mention runs through the same `runAsk` pipeline — **ChatOps replies in Slack/Teams**. That third one is worth stating plainly: an `opinionated` or `collective` voice configured for your own machine will be visible to everyone in the channel the bot answers in. It carries no new content and no new egress — only tone — and it is the owner's own configured voice on the owner's own ChatOps integration, which is why it is not scoped out: doing so would make ChatOps replies the one surface that ignores your persona. It composes with `--devil` (persona directive outermost, `--devil`'s directive innermost, closest to the question). Every `tone` and `voice` value governs register and stance, never what gets said — no persona directive may tell the model to omit content. A `terse` persona can still raise the rate at which a brief falls back to its deterministic rendering, because the honesty contract that protects every brief discards a rewrite that drops a required disclosure rather than risk losing it — that is the contract working, not a bug, and the resolved persona rides the `briefReady` notification's `synthesis` field alongside the discard reason, so a `contract_violation` under `tone = "terse"` is self-explanatory.
...
The fourteenth built-in agent: a cited contribution brief for one person over a time window, assembled entirely from evidence already in the local index — no connector is opened and nothing is fetched live. Seven lanes: PRs authored (with size stats where the enrichment pass has reached them), PRs reviewed (approve / changes-requested / other), tickets opened and tickets closed by an authored PR, code and services owned, decisions authored, documents/notes/messages written, and incident/error-issue attribution — PagerDuty incidents resolved or assigned, and Sentry error issues assigned, reported as two separate counts (an error group that never paged anyone is not an incident). Either connector's absence, or its presence with no attributable edges yet, is reported as a named gap note rather than a false zero.
...
- **On-call shifts and deploys triggered are not available at all.** The local index carries no attribution for either. The brief names both, unconditionally, on every run — never only when they would otherwise read as an empty or zero section, so a genuine zero is never confused with "cannot be computed." Incidents resolved is **not** on this list: the `person --resolves--> incident` graph edge is emitted and the incidents lane measures it directly, the same way Sentry error-issue assignments are measured via the `person --assigned--> error_issue` edge. Either measurement's absence — no PagerDuty/Sentry connector, or a connector with no attributable edges yet — is reported as its own named gap note instead of a silent zero.
...
`packages/gateway/src/multimodal/build-media-pass-deps.ts`), one value for images and audio/video alike — there is
