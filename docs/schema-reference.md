...
-- V60 -- the overnight agent fleet (invariant I38, static rule D28).
-- Authoritative source: packages/gateway/src/index/fleet-v60-sql.ts
--
-- `fleet_job_state` holds ONLY what config cannot. `nimbus.toml`'s [[fleet.job]] blocks stay the
-- source of truth for what a job IS; this table records what HAPPENED to it. Same split as
-- sync/scheduler-state-repository.ts. `backoff_until` is set from `consecutive_failures` by an
-- exponential curve (1 h base, doubling, capped at 24 h) computed in fleet-store.ts, not here.
CREATE TABLE IF NOT EXISTS fleet_job_state (
    job_id               TEXT PRIMARY KEY,   -- the [[fleet.job]] `name`
    last_attempt_at      INTEGER,            -- Unix ms; NULL until the job has been tried once
    last_success_at      INTEGER,            -- Unix ms; drives the interval check, NOT last_attempt_at
    consecutive_failures INTEGER NOT NULL DEFAULT 0,
    backoff_until        INTEGER,            -- Unix ms; the job is not due before this
    last_error           TEXT
) WITHOUT ROWID;

-- One row per RUN. Deliberately SELF-DESCRIBING: every number a run reported must stay derivable
-- from the row alone, months later, after the config that produced it has been edited. That is why
-- `jobs_in_scope` is persisted rather than recomputed from today's config, and why "not due" is a
-- separate column from "attempted".
--
--     unattempted = jobs_in_scope - jobs_attempted - jobs_skipped_not_due
--
-- Column by column:
--   host_power / host_idle_ms / host_source -- the admission probe, verbatim. `host_source` is
--     'measured' or 'power_only'; 'power_only' means user-idle could NOT be measured on this host
--     (Linux always, headless anything) and the run admitted on power alone. `host_idle_ms` is NULL
--     in that case. A run row therefore never implies an idle check that was not performed.
--   outcome -- 'completed' (the whole scope was worked through), 'yielded' (admission flipped
--     mid-run and the scheduler stopped early -- NOT a failure), 'deferred' (admission refused
--     before any job ran), 'failed'. NULL while the run is still open.
--   jobs_in_scope -- the whole configured set, or 1 for a targeted `nimbus fleet run <job>`.
--   jobs_skipped_not_due -- in scope but outside its interval or inside its backoff. Separate from
--     `jobs_attempted` on purpose: a reader must be able to tell "not scheduled yet, working as
...
--     boundary; without that reset a configured 5 would have meant five calls for the gateway's
--     entire lifetime.
...
-- The precomputed brief itself. `run_id`'s cascade is LIVE, not decorative: index/local-index.ts
-- runs `PRAGMA foreign_keys = ON`, and a cascade written against a database that never enabled them
...
-- `brief_markdown` is nullable: a job can complete with findings and no synthesised body.
