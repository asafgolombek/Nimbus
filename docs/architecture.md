...
**Status + dated delivery log:** see [`CHANGELOG.md`](./CHANGELOG.md) (canonical) and [`roadmap.md`](./roadmap.md) (phases + acceptance criteria). Current invariants through I39 (I28 reserved); schema V60.
...
- **Notification contract** — each agent emits exactly one `<agentName>.briefReady { sessionId, brief: string, findings, synthesis }` IPC notification on completion, from the shared `emitBriefWithSynthesis` helper. `brief` is always Markdown; `findings` is that agent's own typed brief object (`ExpertBrief`, `WhyBrief`, …); `synthesis` is the `SynthesisProvenance` union (`agents/_lib/synthesize.ts`) naming whether `brief` is the deterministic render or an LLM-synthesized rewrite, and why not when it isn't (see the `nimbus-agent-patterns` skill for the per-field detail). One documented exception: `agents.whyPeek` answers SYNCHRONOUSLY with a one-line result and emits no notification at all — it does no fan-out, so there is nothing to wait on.
...
//   and ChatOps) and the MCP tool surface — `EXTERNAL_EXCLUDED_AGENT_METHODS` in
...
//   and ChatOps — `EXTERNAL_EXCLUDED_AGENT_METHODS` in `ipc/agents-rpc.ts`) and the MCP tool surface
...
// its own gate rather than a widened `ToolExecutor` action type — the same reasoning I33 already
// established. The gate reuses I33's machinery rather than reimplementing it: the same
// `SandboxRunner`/`SandboxPolicy` from the PAL, the same `ConsentBroker` (a fourth thin binding,
// `cu-consent-broker.ts`, after share/federation-preflight/exec), the same `appendAuditEntry` chain.
//
// computer.sessionOpen  — the ONLY way to open a live envelope. Refuses BEFORE consent when
//   `[computer_use] enabled` is false, when `EnforcedPolicy.capabilitiesDisabled` forbids it (I22),
//   or when the requested lane is absent from `allowed_lanes`; asserts, STILL BEFORE CONSENT, that
//   the EXACT launch policy this session will spawn with is safe (`cu-lanes/browser-launch.ts`'s
//   `assertBrowserLaunchPolicy` — the same object is then handed to `openLane`, whose driver spawns
//   its `argv` verbatim). That replaced a `SandboxRunner.canConfine` assertion over a placeholder
//   policy nothing ever launched with; see `SECURITY-INVARIANTS.md` § I35 for why the browser does
//   not spawn through the PAL at all. Then the LOCAL owner approves the verbatim envelope — lane, the full `navigateOrigins` +
//   `scriptOrigins` lists (never elided/summarised), the action budget, the wall-clock budget —
//   over `computer.envelopeRequest`. Tools are registered on the model only once a session is
//   live; there is no computer-use surface outside one.
// computer.act          — one actuation inside an already-open session. Refuses (never prompts)
//   an action outside the envelope; classifies `observing`/`actuating` STRUCTURALLY from the
//   gateway-observed target (`cu-classify.ts`), never the model's `modelDescription` field (I3
//   transplanted); obtains single-use owner approval over `computer.actionRequest` for every
//   `actuating` verdict; appends one `computer.action` audit row for every outcome, fail-closed,
...
//   REQUESTING side uses a DIFFERENT mechanism, not this one: `platform/assemble.ts`'s
//   `CuGateDeps.requestApproval` switches on the `promptKind` literal discriminant to pick which
//   broker's `.request()` the GATE calls when IT issues a prompt. That was a `"seq" in input`
//   structural probe until 2026-08-31 — sound only because `seq` happened to exist on one shape
//   and not the other, and a `seq` added to the envelope input would have silently routed every
...
// RCE-class, and admitting `computer.approvalRespond` over the wire would let a paired peer
// approve actuations on the owner's machine, defeating I35 without ever calling `computer.act`.
//
// **The driver landed 2026-08-31: raw CDP over a WebSocket, no dependency at all**
// (`computer-use/cu-lanes/`). `platform/assemble.ts` now supplies the four `CuGateDeps` driver
// seams — `resolveBrowserPath` / `buildLaunchPolicy` / `assertLaunchable` / `openLane` — and is the
// ONLY file that may (static D26(c) confines `openBrowserLane` to it plus the driver's own
// definition). `playwright-core` is not reintroduced in any form: it fails a `bun build --compile`
// gate (a statically-resolved `require` inside its bundled, unused WebDriver-BiDi transport, which
// bun's bundler resolves eagerly).
//
// **Read the confinement claim narrowly.** The browser does NOT spawn through `SandboxRunner`, a
// recorded deviation from the design spec's § 3.5: no PAL runner can carry a CDP control channel
// (macOS `(deny default)` denies `network-bind`; the Windows AppContainer helper forwards no fds
// 3/4; Linux and Windows both need `nimbus-sandbox-helper` for any network-bearing policy, which CI
// does not install). It is confined instead by Chromium's own multi-process sandbox — kept intact,
// and the pre-consent launch assertion over the EXACT spawning argv is what keeps it intact — a
// Nimbus-owned `--user-data-dir`, the § 3.5.1 CDP request policy, and headless +
// `Browser.setDownloadBehavior: deny`. `nimbus computer browser` remains a PASSIVE LISTENER by
// design: it opens a session and answers its two prompt kinds, while `computer.act` is driven from
// inside the gateway by `cu-tools.ts`'s model-callable tools, never over this RPC from the CLI.
// The browser lane's own egress class (`egress/browser-egress.ts`'s `wrapLedgeredBrowserContext`,
// a decorator over the CDP-backed context `openBrowserLane` constructs) is consequently LIVE:
...
// --- Multimodal I/O (Spine S2; local media understanding, V59 `media_grant` — artifact-scoped remote-model consent) -------------
...
// fleet.briefs  — precomputed briefs, newest first. `limit` defaults to 20 and is capped at 500
//   HERE, at the IPC boundary rather than in the CLI: the CLI is not the trust boundary and a
//   `brief_markdown` body can be tens of KB.
...
// rather than by remembering. `fleet/` shape: `fleet-admission.ts` (the pure probe→verdict
// predicate — blocks on `battery`, does NOT require `ac`, because a desktop/VM/server answers
// `unknown` and an `=== "ac"` predicate would make the feature silently inert on exactly the
// always-on hardware it targets), `fleet-scheduler.ts` (60 s tick, single-flight `inFlight` guard
// over every entry path, per-job interval + 1 h-base exponential backoff capped at 24 h, yields
// mid-run when admission flips), `fleet-invoker.ts` (dispatches through `dispatchAgentsRpc` under
// the gateway-set `fleet` `ClientKind` and awaits the completion NOTIFICATION, not the call — the
// call returns before the brief exists), `fleet-store.ts` (V60), `fleet-synthesis-router.ts`
// (I38's decorator over `SynthesisRouter`, guarding `resolveForSynthesis` AND `generateMarkdown`).
// `EGRESS_BEARING_CLIENT_KINDS.fleet` is `null` — the same `null` as `cli`/`ui`, not an oversight:
// a fleet brief is local SQLite, and a GRANTED remote synthesis is ledgered by the `model` class at
// the provider, where the bytes actually leave.
```
...
The full table-by-table SQL — the unified V3 `item`, `item_fts`, `vec_items_384` / `vec_items_1536`, `audit_log`, `sync_state`, `connector_health_history`, `api_endpoint`, `obsidian_notes`, the latency/slow-query logs, `llm_models`, `sub_task_results`, `tool_call_log`, `extension_dependency`, `git_blame_line`, `extensions`, the Phase 6 federation / org-policy / share tables, the V44 `egress_ledger`, the V45 `glossary_term` / `glossary_pass_state` tables, the V46 rebuild of `glossary_term` (widened `definition_source` CHECK), and the V57 `cu_session` / `cu_action` computer-use tables — lives in **[`schema-reference.md`](./schema-reference.md)**. It was extracted from this document so the architecture narrative stays focused on shape rather than every column; it grows with every migration.
...
- The Phase 6 **S1 "Local Brain" egress ledger** (`egress_ledger`, V44) is the always-on, append-only, BLAKE3-chained record of authorized outbound **connector** actions, written from `engine/executor.ts` `ToolExecutor.gate()` **before** `connectors.dispatch` (invariant `I29` / static `D22`): a denied gate records a `result_status='blocked'` row and an append failure aborts the action (fail-closed, never dispatches), so a `0`-row window *from a wired, dispatch-capable executor* is a sound negative. `destination` is the `serviceOf()` action-type prefix (never a raw URL); `payload_summary` is `redactAuditPayload`-scrubbed and capped at 256 bytes (a debugging aid, NOT the security boundary). The sole mutation is the HITL-gated `egress.prune` continuing tombstone; receipt signing reuses the Vault-only Ed25519 share keypair (no new Vault key). `nimbus prove "<query>"` counts rows over `[since, until]` via `egress.proveWindow`, never a raw ledger-head diff (see below — that would also count `blocked` rows and boot markers); `nimbus egress [verify|prune]` is the report / offline chain-verify / retention surface.
  - **Read the scope before quoting the guarantee** — [`SECURITY-INVARIANTS.md` § I29](./SECURITY-INVARIANTS.md#i29--egress-ledger-completeness-over-the-executor-chokepoint) is authoritative and states it precisely; this bullet is the map, not the contract. Three qualifiers travel with it. (1) The seven **gate-only** executors (vault, teamvault, reindex, data, auto-update, `connector.auth`, `egress.prune`) pair with a rejecting dispatcher and perform local mutations, not egress — they are wired with the named `NULL_EGRESS_SINK` and intentionally record nothing. That is a documented exclusion, not a coverage gap. (2) **D22's rule (a) is a regex over the literal string `connectors.dispatch`** — a confinement check, not a proof that every path to the network is ledgered. A dispatcher decorator calling `inner.dispatch(...)`, a façade re-exposing execution under another method name, and a raw `tool.execute()` on a lazy-mesh tool record all pass it. None are exploited today; closing them is capability removal, not a stronger regex. (3) The machine-readable coverage claim is the **coverage vector** (`egress/egress-coverage.ts`), not this prose: `task` is `per-call` (the executor's gated-action append), `mcp` and `http` are both `per-call` (`egress/agent-brief-egress.ts` — an agent brief served to an MCP-connected client or to a token-verified caller on the local HTTP API, appended from `ipc/agents-rpc.ts` before any agent work; D22 rule (c) pins that single caller and rule (d) forbids any other file importing an agent emitter, so a second entry point cannot bypass it — `http` covers agent briefs only, NOT the rest of the HTTP read surface), `sync` is **`per-run`** (raised from `none`; `packages/gateway/src/egress/sync-egress.ts`'s `recordSyncEgress`, shared by FOUR callers — `packages/gateway/src/sync/scheduler.ts`'s per-RUN scheduled-sync append, `packages/gateway/src/sync/targeted-fetch.ts`'s per-CALL targeted-fetch append, `packages/gateway/src/multimodal/cloud-url-resolver.ts`'s append before the CREDENTIALED byte-URL resolve round-trip (`method='media.resolveByteUrl'`), and `packages/gateway/src/multimodal/cloud-bytes.ts`'s per-ATTEMPT cloud byte-fetch append (`method='media.fetchBytes'`) for a Google Photos/Drive/OneDrive media artifact (the last two share ONE closure wired by `packages/gateway/src/multimodal/build-media-pass-deps.ts`, not `packages/gateway/src/platform/assemble.ts`, so a Photos/OneDrive candidate yields TWO rows for its two real outbound requests while a Drive candidate — whose URL is constructed with no round-trip — yields one) — `per-run` is the honest, weakest of the four shapes it backs, and only for a CONFIGURED connector's syncable: `packages/gateway/src/sync/connector-configured.ts`'s `isConnectorConfigured` gates the scheduler's append, not the call, so an unconfigured connector's `sync()` still runs unledgered), `model` is **`per-call`** (`egress/model-egress.ts`'s `wrapLedgeredProvider` — one appender, a DECORATOR applied at `LlmRegistry.addRoute`, so it covers every generate on a NON-LOCAL route: `LlmRouter.generate()`, `generateMarkdown()` and every `selectProvider()` caller alike, without any of them cooperating; not literally "all inference" over every model call this binary can make, but it now carries no NAMED exclusion. The `ask` intent classifier JOINED this class on 2026-08-28, and is the reason the count was briefly ONE and not two: until then `engine/router.ts` held its own HTTP client, read `ANTHROPIC_API_KEY`/`OPENAI_API_KEY` straight from the process environment and POSTed the user's question to a vendor with no row appended and no `[llm.remote.*]` opt-in — an UNDISCLOSED exclusion rather than a stated one, so `nimbus prove` reported `0` for a query that had made a real outbound request carrying user text. It now asks `LlmRouter` for the `"classification"` task, so `wrapLedgeredProvider` covers it like any other route and it appends `method='engine.ask.classify'`. The Mastra engine agent, previously the second exclusion, is covered as of slice 2b by a SECOND appender — `egress/mastra-model-egress.ts`'s `wrapLedgeredMastraModel`, decorating its model at the AI-SDK seam — and is not constructed at all when no `[llm.remote.*]` vendor is enabled; the local-vs-remote split is enforced INSIDE the wrapper, which DERIVES it from `provider.isLocal` rather than trusting a caller-computed boolean, the same choice `recordSyncEgress` makes for `LOCAL_ONLY_SYNC_SERVICES`, and the `isLocal` declaration it reads is pinned by invariant I34; static rule D22(e) confines `registerRoute` to `llm/registry.ts` so nothing enters the route table unwrapped. Embeddings, previously the LAST exclusion, are covered by a THIRD appender — `egress/embedding-egress.ts`'s `wrapLedgeredEmbedder`, a DECORATOR applied at the embedding pipeline's three construction sites (`embedding/create-routing-runtime.ts`, `embedding/create-embedding-runtime.ts`, `ipc/index-reembed-rpc.ts` — confined there by static rule D22(f)) rather than at one call site, so it covers every `embed()` caller including ones written later; it appends ONE row per embed BATCH, not per text (`method='embedding.embed'`), before `PROSE_HEAVY_TYPES` prose reaches OpenAI's 1536-dim table, and a LOCAL embedder (MiniLM) is still returned UNCHANGED and appends nothing — the class working as designed, not a residual gap. So a `model: 0` window now means no non-local generate, no non-local embed AND no non-local vision describe left the machine — `egress/vlm-egress.ts`'s `wrapLedgeredVlm` (D22(g)) is the fourth appender in this class, added by the multimodal vision slice. **Do not read the class list here as the roster:** `browser` (`per-run`) and `chatops` (`per-call`) are live too, and only `session` and `peer` are `none`. This paragraph is prose and has drifted before — derive the authoritative set from `THIS_BINARY_COVERAGE` in `packages/gateway/src/egress/egress-coverage.ts`, never from a sentence. It is carried in the hashed `source_id` of a per-process **boot marker** (`egress/egress-boot-marker.ts`, appended non-fatally by `platform/assemble.ts`), so it cannot be edited without breaking the chain — and a window with no covering boot marker reports `indeterminate`, never a clean `0`. Marker rows (`prune`/`boot`/`degraded`) are bookkeeping and are excluded from the outbound count. `egress.proveWindow` (backing `nimbus prove`) counts only **authorized, non-marker** rows inside `[since, until]` (`outboundEgressEvents`) — never a raw `egress.head` before/after count, which would also count `blocked` rows and `boot`/`degraded` markers — alongside a whole-ledger (`fromId=0`) chain verify, so a `0` is reported only when both the count is zero and the chain is intact.
- **The migration runner** at [`packages/gateway/src/index/migrations/runner.ts`](../packages/gateway/src/index/migrations/runner.ts) is authoritative (`INDEXED_SCHEMA_STEPS`). **Latest applied migration: V60** (`fleet-v60-sql.ts` adds `fleet_job_state` / `fleet_run` / `fleet_brief` — the overnight agent fleet's durable state: what happened to each configured job, one self-describing row per run, and the precomputed briefs themselves. Config stays the SSoT for what a job IS; these tables record what HAPPENED, the same split `sync/scheduler-state-repository.ts` uses. `fleet_run` persists `jobs_in_scope` rather than recomputing it, so `unattempted = jobs_in_scope - jobs_attempted - jobs_skipped_not_due` still resolves after the config that produced the run has been edited, and it records the admission probe verbatim (`host_power` / `host_idle_ms` / `host_source`) so a run never implies an idle check that was not performed — `host_source = 'power_only'` is the honest answer on Linux, always. `fleet_brief.run_id` cascades, which is live rather than decorative because `index/local-index.ts` runs `PRAGMA foreign_keys = ON`. All three `WITHOUT ROWID`. New invariant **I38**, static rule **D28** — S2 overnight agent fleets, PR 1 of 2). **V59** (`media-grant-v59-sql.ts` adds `media_grant` — one row per (artifact, modality, vendor) grant of consent to send an image to a remote vision model; `WITHOUT ROWID`; `modality` CHECK'd to `image`/`av`; the partial unique index `idx_media_grant_active ... WHERE revoked_at IS NULL` is load-bearing, since it permits re-granting after revocation while forbidding two simultaneous active grants for the same triple — revocation stays an append-only audit trail rather than being terminal. S2 multimodal I/O, PR 4 of 4. No new invariant). **V58** (`media-pass-v58-sql.ts` adds `media_pass_cursor` — one row per multimodal understanding pass (`pass_id` PK, nullable `service`/`modality` scope, `last_item_id`, `processed_count`, `updated_at`), `WITHOUT ROWID`. SQLite-backed rather than in-memory precisely because a budgeted pass over a large media library must resume across a gateway restart, which is the whole point of budgeting it. The per-root media GRANTS were deliberately deferred out of V58 and land in V59 (`media-grant-v59-sql.ts`), because a forward-only schema makes a table created three PRs before anything reads it drift waiting to happen. No new invariant — S2 multimodal I/O, slice 1 of 4. **V57 is the computer-use pair** (`computer-use-v57-sql.ts`): `cu_session` — one row per computer-use session: `lane` CHECK'd to `browser`/`terminal`/`screen`, of which `browser` and `terminal` both have a gate AND a driver (the raw-CDP browser driver landed 2026-08-31, the pipe-backed terminal lane 2026-09-01) and `screen` has neither — the approved `envelope_json` verbatim, `opened_at`/`closed_at`/`close_reason`, and the taint-latch `tainted_at` (NULL until the first untrusted observation, monotonic once set) — and `cu_action` — one row per actuation, `UNIQUE (session_id, seq)`, `session_id REFERENCES cu_session(id) ON DELETE CASCADE`, `classification` CHECK'd to `observing`/`actuating`. `observed_target` (what the classifier read — a gateway-derived fact) and `model_description` (what the model said it was doing — untrusted, forensic-only) are DELIBERATELY separate columns: collapsing them would destroy the one distinction invariant `I35`'s consent prompt turns on. `dom_before`/`dom_after` hold the browser lane's before/after DOM snapshot, truncated above `[computer_use] snapshot_max_bytes` with `dom_truncated`/`dom_original_bytes` recording the clip so a reader can never mistake a truncated snapshot for a complete one — the same `truncated` convention `exec` already uses; `[computer_use] snapshot_retention_days` (default 7) NULLs both columns past the window on a daily sidecar pass, while the audit-log decision row survives permanently. `screenshot_digest` is the ONLY screenshot-shaped column on either table — a BLAKE3 digest, never pixels, since screenshot bytes are hashed and discarded in the same expression that captures them and are never written to disk on any lane, at any point (S2 computer-use, invariant `I35`, static `D26`) — see [`SECURITY-INVARIANTS.md` § I35](./SECURITY-INVARIANTS.md#i35--a-computer-use-actuation-reaches-the-host-only-inside-an-owner-approved-envelope). Both tables are `WITHOUT ROWID`. **V56 is not narrated here** — `connector-configured-v56-sql.ts` adds `sync_state.configured`, so a connector nobody configured stops reporting itself `healthy`; see `schema-reference.md`'s V56 entry for detail — S2 computer-use / local-LLM audit. V55 adds `pr_changed_file` + `pr_files_state` — PR changed-file paths plus a per-PR fetch-coverage record, both keyed on `item.id` with `ON DELETE CASCADE`, both `WITHOUT ROWID` — one row per touched path (a rename writes two rows, a deletion writes one; `status` is descriptive only), plus a `local_file_id` FK (`REFERENCES graph_entity(id) ON DELETE SET NULL`) that ships **unpopulated** — the ownership pass that would fill it is deliberately deferred, since nothing in this delivery reads it, negation matching on `path` alone. Three forge mappers (`mapGithubPrFiles` / `mapGitlabMrFiles` / `mapBitbucketPrFiles`) plus a shared bounded per-tick driver (`runPrFilePass` — up to 10 PRs RECORDED per tick per service, attempting up to 30 candidates so a permanently-failing newest-first head cannot pin coverage at zero) land the rows; `nimbus status` gains a `PR file coverage: <covered> / <totalPrs>` line, omitted when no PRs are indexed. Ships the fail-closed negation primitive (`selectPrsNotTouching`) for a later PR (W6-B) to call — no `--negate`/`--touches` CLI surface lands here. Coverage grows over many sync ticks, not on one sync. No new IPC method (`ALLOWED_METHODS` stays 105) — S1 "Local Brain". V54 added no new table; it namespaces `graph_entity.metadata` per writer for the six co-owned entity types (`source_file` / `directory` / `person` / `service` / `workspace` / `repo`) via `ENTITY_METADATA_V54_SQL`, wrapping an existing value as `{"ownership": <existing>}` only where it is non-null, `json_valid`, a JSON **object** and not already namespaced — a malformed or scalar value is left untouched, not wrapped — fixing a last-writer-wins bug where `graph/graph-populator.ts`'s code-symbol sync silently NULLed the owner counts `ownership/ownership-pass.ts` had just written on `source_file` (both writers share the same byte-identical `file:<repoRoot>:<path>` entity, deliberately), so `nimbus owners` alternated between its real output and an "owner breakdown not recorded" line depending on which pass ran last, with no error and no gap note; both writers now go through the new `upsertGraphEntityNamespaced`, which merges via a two-step `json_patch` (a `null` patch to clear the writer's own namespace, then a set patch to replace it) rather than the flat `metadata = excluded.metadata` the bug came from; the six-type set is chosen rather than derived — four converge on identical external ids, `directory` has a single writer and is included for uniformity, and `service`'s two writers key disjoint id spaces and are namespaced defensively (per-type rationale lives at `CO_OWNED_ENTITY_TYPES`); a static audit rule rejects a flat `upsertGraphEntity` write on a co-owned type outside `relationship-graph.ts` itself, with two bounds it states openly — `.test.ts` files are exempt, and it (like the `NonCoOwnedType<T>` compiler guard) resolves literal `type:` arguments only — S1 "Local Brain"; V53 added `premortem_theme` / `premortem_theme_evidence` / `premortem_pass_state` / `premortem_watcher_proposal` — the schema for a debounced background pass that mines recurring blocker themes per service from closed epics (discover → extract via local LLM → reconcile), checkpointed by a composite `(modified_at, id)` watermark; `premortem_watcher_proposal` was written by a later PR, not the migration itself — the table landed here because schema precedes its reader; **all four V53 tables now have a reader** — `nimbus pre-mortem` / `agents.premortem` shipped 2026-08-11, reading `premortem_theme` + `premortem_theme_evidence` and writing `premortem_watcher_proposal` (paused watchers only, and only for services that resolve to a configured `[ci.service.<id>]` id — see the `nimbus-agent-patterns` skill); the discover stage is **Jira-only today** (keys on `metadata.issue_type = 'Epic'`, written only by `jira-sync.ts` — no `linear:project` items are indexed, so there is no Linear epic-shaped row to mine) — S1 "Local Brain"; V52 added `item.resolve_key` (`canonicalizeUrl(canonical_url ?? url)`, NULL when both `canonical_url` and `url` are null) + `idx_item_resolve_key`, derived at the `upsertIndexedItem` SQL write site — the chokepoint every connector's item write funnels through, so no connector can forget it (`deployment/annotate.ts` is a second, non-connector `item` writer and derives the same key the same way) — and backfilled row-wise inside the migration; matched by the bounded ladder behind `GET /v1/items/resolve` — S1 "Local Brain"; V51 added the ownership relation types (`owns` / `contains` / `tracks_remote`) + `ownership_pass_state` — the ownership graph derived from git blame — S1 "Local Brain"; V50 is a deliberate no-op step (it bumps `user_version` and writes a ledger row, nothing else) — it is NOT available to backfill later, because the runner applies a step only while `user_version === fromVersion`, so a database already at V51 never re-enters it; parallel work must take a NEW version; V49 (`depth-default-v49-sql.ts`) backfills `sync_state.depth` `'summary'` → `'full'`, deliberately leaving `metadata_only` rows untouched, so the per-connector depth setting is real before connector-index-depth enforcement starts honouring it; V48 added `item.body` + `item.body_complete`, with `item_fts` repointed from `body_preview` to `body` — the full-body store, S1 "Local Brain"; V47 added `decision_record` + `decision_evidence` + `decision_pass_state` — the implicit ADR extractor — S1 "Local Brain"; V46 was a full-table rebuild of `glossary_term` widening `definition_source` from `CHECK(... IN ('llm','snippet'))` to `CHECK(... IN ('llm','snippet','manual'))` for manual term authoring — SQLite cannot alter a CHECK in place, and V45 had already shipped in v1.13.0, so the table is rebuilt rather than edited; columns are copied by name, not position — S1 "Local Brain"; V45 added `glossary_term` + `glossary_pass_state` — the implicit-knowledge glossary's SSoT + single-row watermark — S1 "Local Brain"; V44 added `egress_ledger` — the always-on, BLAKE3-chained provable-locality ledger written before `connectors.dispatch` — Phase 6 S1 / invariant `I29`; V43 added `share_inbox` deferred-reveal inbox — Phase 6 Slice 8d; V42 added `tool_call_log.params_json` recipe step params — Phase 6 Slice 8b; V41 added `share_records`, the content-addressed share ledger — Phase 6 Slice 8a; V40 added `graph_relation_type` cross-warehouse lineage relations — `derived_from` / `upstream_refs` / `monitors` — Phase 6 Slice 7 Wave 7a; V39 added the `tribal_clusters` repeated-question cluster ledger — Phase 6 Slice 6c; V38 added the asker-side known-namespaces cache — Phase 6 Slice 6a; V37 added the GDPR-purge ledger — Phase 6 Slice 4; V36 added the org-policy tables — Phase 6 Slice 4; V35 added team-vault entries/grants + quorum delegations — Phase 6 Slice 2; V34 identity/SCIM tables — Phase 6 Slice 3; V33 added federation namespaces/filters/grants + `audit_log.federation_json` — Phase 6 Slice 1; `CURRENT_SCHEMA_VERSION = 60` in `index/local-index.ts`). Migrations are append-only and forward-only — see the [`nimbus-db-migrations`](../.claude/commands/nimbus-db-migrations.md) skill for the authoring contract.
...
| Read-only HTTP API | `ipc/http-server.ts` | `SQLITE_OPEN_READONLY` + `PRAGMA query_only = ON`; bearer-gated read routes incl. `GET /v1/admin/status`, `GET /metrics` (Prometheus exposition), `GET /admin/*` (the `packages/admin-console` static console — Phase 6 Slice 4), `GET /v1/items/resolve` (S1 resolve-by-URL; `resolve` clip-token scope; answers from the local index only — never an outbound request, matched by the bounded ladder over the V52 `item.resolve_key`), `GET /v1/items/resolve-file` (C7 forge-file resolution; same `resolve` clip-token scope; maps a `{service, repo, refAndPath}` coordinate to a repo-relative path via the `tracks_remote` graph edge, answering from the local index only — never an outbound request, and never returning the local `repoRoot`), `GET /v1/items/resolve-ids` (same `resolve` clip-token scope; the only one of the three that takes a **list** rather than one reference — up to `RESOLVE_IDS_MAX_BATCH = 100` raw `?id=` parameters, counted before de-duplication and refused with `400 too_many_ids` rather than clamped — mapping item ids that appear inside an agent brief back to `{ id, service, type, title, url, modified_at }`, with an id the index does not hold simply absent from the response rather than a null entry, and again answering from the local index only — never an outbound request) |
...
The canonical invariant table (currently **I1–I39**, with I28 reserved) lives in [`SECURITY-INVARIANTS.md`](./SECURITY-INVARIANTS.md) — each row names the defense, its production wiring site, the anti-pattern that regresses it, and the enforcement test. It is deliberately **not** duplicated here: a third copy (alongside the compact summaries in `CLAUDE.md` / `GEMINI.md`) is how it drifts. When changing a wiring site, update the invariants file *and* the enforcement test in the same commit.

A static-time complement (`scripts/structure-audit/check-nimbus-invariants.ts`) catches I1 (`spawn` under `connectors/` must use `extensionProcessEnv()`), the vault-key allow-list, I14 (`D12` — direct `db.run`/`db.exec` outside `db/write.ts` exits 1), I15 (`D10` — every `ServerSpec` under `connectors/lazy-mesh/` must pass through `wrapServerSpec(...)`), I17 (`D13` — federation modules other than `query-gate.ts` may not import the item-list query), I18 (`D14` — identity-token Vault-key literals are forbidden outside `identity/`), I19 (`D15` — the `teamvault.` Vault-key prefix is composed only in `team-vault-keys.ts`), I22 (`D16` — `parsePolicyToml` may be imported only under `policy/`), I23 (`D17` — the `slack_chat_post` / `teams_chat_post` operational-post tools are referenced only from `chatops/reply-dispatcher.ts` and the transport), I24 (`D18` — a federated preflight action resolves its command from local config only and runs behind the local owner's HITL gate, scoped to `federation/preflight-gate.ts`), I25 (`D19` — the `notion_kb_append` / `confluence_kb_append` tribal-KB write tools are confined to `tribal/tribal-write-gate.ts` and the connector), I26 (`D20` — connector write tool ids (warehouse/BI ∪ GitOps/ML) are confined to their SSoT + connector + transport sites and rejected by the federated invoke gate), I27 (`D21` — an outbound share emit + the `createShare()` call are confined to `share/share-gate.ts` and `ipc/share-rpc.ts`), I29 (`D22` — seven rules: `connectors.dispatch` confined to `engine/executor.ts`, the `egress_ledger` append to `egress/*`, `recordAgentBriefEgress` pinned to its single caller, the `agents/<name>.ts` emitters unimportable outside `ipc/agents-rpc.ts`, `registerRoute` confined to `llm/registry.ts`, `wrapLedgeredEmbedder` to `egress/embedding-egress.ts` plus its three construction sites, and `wrapLedgeredVlm` to `egress/vlm-egress.ts` plus `multimodal/build-media-pass-deps.ts` — keeping the egress chokepoints total), I33 (`D23` — `runConfined` is confined to `exec/exec-gate.ts`, so user-supplied code has one spawn path), I35 (`D26` — three rules: `performActuation` confined to the computer-use gate, the driver capability (an automation-library import **or** a raw CDP `Domain.method` literal) confined to `computer-use/cu-lanes/`, and each lane constructor nameable only by its own definition plus `platform/assemble.ts`), I37 (`D27` — two rules: the non-local `VlmProvider` CONSTRUCTOR confined to `multimodal/build-media-pass-deps.ts` inside a `wrapLedgeredVlm(...)` argument list, and the `media_grant` table confined to `multimodal/media-grant-store.ts`, `multimodal/media-discovery.ts`'s correlated read, and `index/media-grant-v59-sql.ts`'s DDL), I38 (`D28` — the `fleet` `ClientKind` ASSIGNMENT shapes confined to `ipc/server/client-kind.ts`, `egress/egress-bearing-kinds.ts` and `fleet/fleet-invoker.ts`; it deliberately does NOT match a bare `"fleet"` string, which is a legitimate namespace token in the LAN forbid-list and the CLI command map, and it states its own residual bound — an indirection or a computed key evades it, so capability confinement, not the regex, is the primary defense), I39 (`D29` — three rules: the `nimbus/fetch` brokered-method literal confined to `toolgen/toolgen-types.ts`, `buildGeneratedManifest` confined to `toolgen-stub.ts` with a static ban on a non-empty `network:` literal there, and the `toolgen.` Vault-key prefix confined to `toolgen-credentials.ts`), and the SyncContext capability boundary (`D24`) at audit time. The runtime tests in `packages/gateway/src/security-invariants.test.ts` remain authoritative for invariant wiring; the static checks just catch regressions before the tests run.

This list is COMPLETE as of I38 — it is checked against `scripts/structure-audit/check-nimbus-invariants.ts` rather than appended to by habit. An invariant with no static rule is absent here on purpose: most are runtime-only, and `D`-numbers exist only where a source-scannable shape can catch a regression before the tests run.
...
| Extension sandbox escape | Per-OS sandbox runner intrinsic to every spawn (invariant `I15`, Phase 5 T2 PR 1): bwrap + seccomp + per-host iptables on Linux, sandbox-exec SBPL on macOS, AppContainer + orphan-reap on Windows (reap wired at `platform/assemble.ts` boot; see `platform/sandbox/{orphan-reap,win32-reap}.ts`). Per-host network filtering is full on Linux/macOS and all-or-nothing on Windows ([sandbox.md](./sandbox.md#platform-asymmetry)) | Extension Registry / sandbox PAL |
